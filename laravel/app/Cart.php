...
class Cart extends Model {
    ...
    public static function getCart($sessionId) {
        return $cart = DB::table('cart as c')
                ...
                ->select('c.id as cart_id','c.image as image', 'c.quantity as quantity', 'p.price as total_price', 'p.id as product_id', 'p.name as product_name', DB::raw('group_concat(cpa.attribute_id) as attribute_id'), DB::raw('group_concat(cpa.value_id) as value_id'), DB::raw('group_concat(a.name) as attribute'))
                ->groupBy('c.id')
                ->orderBy('c.id', 'desc')
                ->get();
    }
