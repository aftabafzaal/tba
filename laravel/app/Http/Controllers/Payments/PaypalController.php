...
class PaypalController extends Controller {
    ...
    public function success() {
        ...
        if (count($orders) > 0) {
            ...
            $model->referenceId = $_GET['paymentId'];
            $model->gateway = 'paypal';
            $model->type_id = $order_id;
            $model->type = 'order';
            $model->amount = $orders->grandTotal;
            $model->created_at = date('Y-m-d H:i:s');
            $model->save();

            $modelPaypal = new Paypal();
            $modelPaypal->transaction_id = $model->id;
            $modelPaypal->paymentId = $_GET['paymentId'];
            $modelPaypal->token = $_GET['token'];
            $modelPaypal->payerID = $_GET['PayerID'];
            $modelPaypal->save();
